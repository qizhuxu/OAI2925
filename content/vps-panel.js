...
async function step9_vpsVerify(payload) {
  ...
  try {
    ...
    // 检测超时错误
    if (/timeout|超时|Timeout waiting for OAuth callback/i.test(statusText)) {
      log(`Step 9: OAuth callback timeout detected: "${statusText}"`, 'error');
      throw new Error('OAuth callback timeout - need to refresh OAuth link');
    }
    
    // 检测认证失败
    if (/认证失败|authentication failed|失败/i.test(statusText)) {
      log(`Step 9: Authentication failed: "${statusText}"`, 'error');
